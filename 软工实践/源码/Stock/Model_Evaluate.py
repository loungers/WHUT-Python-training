...
def model_eva(stock,state_dt,para_window,para_dc_window):
    ...
    for d in range(len(model_test_date_seq)):
        model_test_new_start = (datetime.datetime.strptime(model_test_date_seq[d], '%Y-%m-%d')
    ...
    db.commit()
    db.close()
    print('stock:'+ stock +str(state_dt) + '   Precision : ' + str(acc) + '   Recall : ' + str(recall) + '   F1 : ' + str(f1) + '   Acc_Neg : ' + str(acc_neg))
    return 1